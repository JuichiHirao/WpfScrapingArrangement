...
namespace WpfScrapingArrangement.collection
{
    class FileGeneTargetFilesCollection
    {
        ...
        public static string REGEX_MOVIEONLY_EXTENTION = @".*\.avi$|.*\.wmv$|.*\.mpg$|.*ts$|.*divx$|.*mp4$|.*asf$|.*iso$|.*mkv$|.*\.m4v|.*\.rmvb|.*\.rm|.*\.mov|.*\.3gp|.*\.qt|.*\.flv";
        ...
        public void Execute()
        {
            ...
            ColViewListTargetFiles.Filter = delegate (object o)
            {
                ...
                foreach(string searchText in FilterListProductNumber)
                {
                    if (searchText.IndexOf("_", StringComparison.Ordinal) >= 0)
                    {
                        string[] splits = searchText.Split('_');

                        bool isMatch = true;
                        foreach (string split in splits)
                        {
                            if (filename.ToUpper().IndexOf(split, StringComparison.Ordinal) < 0)
                            {
                                isMatch = false;
                                break;
                            }
